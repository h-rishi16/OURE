...
class ManeuverPropagator(BasePropagator):
        ...
        if target_epoch >= state.epoch:
            for man in self.maneuvers:
                if current_state.epoch < man.burn_epoch <= target_epoch:
                    ...
                    current_state = self._base.propagate_to(
                        current_state, man.burn_epoch
                    )
    ...
    def propagate_many_to(
        ...
        # Fallback to loop over individual states since maneuvers are unique per satellite.
        out_states = np.empty_like(states)
        for i in range(len(states)):
            sv = StateVector(
                r=states[i, :3], v=states[i, 3:], epoch=initial_epoch, sat_id="TBD"
            )
            res_sv = self.propagate_to(sv, target_epoch)
            out_states[i, :3] = res_sv.r
            out_states[i, 3:] = res_sv.v
        return out_states
