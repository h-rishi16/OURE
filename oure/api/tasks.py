...
def watchlist_screening_task(self: Any) -> None:
    from oure.monitoring.alerting import AlertDispatcher
    from oure.monitoring.watchlist import WatchlistMonitor

    # We assume 'assets' could be fetched from environment variable or database
    # For now, let's hardcode ISS (25544) as an example asset.
    assets_str = os.getenv("OURE_WATCHLIST_ASSETS", "25544")
    assets = [a.strip() for a in assets_str.split(",") if a.strip()]

    if not assets:
        return

    monitor = WatchlistMonitor(assets=assets)
    alerts = monitor.get_alerts()

    webhook_url = os.getenv("OURE_SLACK_WEBHOOK_URL")
    dispatcher = AlertDispatcher(webhook_url=webhook_url)

    for alert in alerts:
        if alert.alert_level in ("ACTION", "MONITOR"):
