...
def _run_baseline_assessment(
    p_state: StateVector,
    s_state: StateVector,
    p_cov: CovarianceMatrix,
    s_cov: CovarianceMatrix,
    flux: float,
    primary: str,
    secondary: str,
) -> tuple[ConjunctionEvent | None, NumericalPropagator]:
    with console.status("[bold cyan]Running baseline High-Precision propagation..."):
        ...
        tca_result = tca_finder.find_tca(
            ...
            base_prop,
            search_start,
            search_start + timedelta(hours=72),
        )
    ...
    return nominal_event, base_prop
...
def _run_trade_space(
    ...
    with console.status("[bold cyan]Simulating trade space..."):
        tca_finder = TCARefinementEngine()
        risk_calc = RiskCalculator()
        burn_state = base_prop.propagate_to(p_state, burn_epoch)
        v_hat = burn_state.v / np.linalg.norm(burn_state.v)

        for dv_cm in [-5.0, -1.0, -0.5, -0.1, 0.1, 0.5, 1.0, 5.0]:
            maneuver = Maneuver(
                burn_epoch=burn_epoch, delta_v_eci=v_hat * ((dv_cm / 100.0) / 1000.0)
            )
...
def avoid(
    ...
    s_cov = _default_covariance(secondary)

    nominal_event, base_prop = _run_baseline_assessment(
        p_state, s_state, p_cov, s_cov, flux, primary, secondary
    )
    if not nominal_event:
        return

    burn_epoch = nominal_event.tca - timedelta(hours=burn_time_before_tca)

    if optimize or Confirm.ask(
        "[highlight]Run mathematical optimization for minimum-fuel burn?[/highlight]"
    ):
        _run_optimization(
            p_state, s_state, p_cov, s_cov, burn_epoch, base_prop, burn_time_before_tca
        )
    else:
        _run_trade_space(
            p_state,
